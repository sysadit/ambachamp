'use client';
// src/app/penyelenggara/lomba/[id]/edit/page.jsx
...
export default function EditLombaPage() {
  ...
  useEffect(() => {
    ...
    lombaAPI.getById(id)
      .then((r) => {
        const d = r.data.data;
        reset({
          judul: d.judul, deskripsi: d.deskripsi, kategori: d.kategori,
          tingkat: d.tingkat, biaya_pendaftaran: d.biaya_pendaftaran,
          deadline_pendaftaran: d.deadline_pendaftaran?.split('T')[0],
          tanggal_pelaksanaan: d.tanggal_pelaksanaan?.split('T')[0] || '',
          link_pendaftaran: d.link_pendaftaran || '',
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Gagal memperbarui lomba. Coba lagi.');
    } finally {
      setLoading(false);
    }
  };
  ...
  if (fetching) return (
    <DashboardLayout title="Edit Lomba">
  ...
  if (success) return (
    <DashboardLayout title="Edit Lomba">
      <div className="card p-10 text-center max-w-md mx-auto">
        ...
        <h3 className="font-semibold text-slate-800 mb-1">Lomba Berhasil Diperbarui!</h3>
        <p className="text-sm text-slate-500">Perubahan akan diverifikasi ulang admin. Mengarahkan ke dashboard...</p>
      </div>
    </DashboardLayout>
  );
      ...
      <div className="max-w-2xl">
        <div className="card p-6 mb-4 bg-amber-50 border-amber-200">
          <p className="text-sm text-amber-800">
            Setelah diedit, lomba akan kembali berstatus menunggu verifikasi admin sebelum tampil lagi.
          </p>
        </div>
